  ...
  async getPlayoffGrid(tournamentId) {
    try {
      ...
      return {
        ...
        textBlocks: (data.text_blocks ?? []).map((block: any) => ({
          id: String(block.id),
          col: Number(block.col),
          row: Number(block.row),
          widthCells: Number(block.width_cells ?? 1),
          heightCells: Number(block.height_cells ?? 1),
          text: String(block.text ?? ''),
          visible: block.visible !== false,
          showBackground: block.show_background !== false,
          align: (['left', 'center', 'right'].includes(String(block.align)) ? String(block.align) : 'left') as 'left' | 'center' | 'right',
          verticalAlign: (['top', 'center', 'bottom'].includes(String(block.vertical_align)) ? String(block.vertical_align) : 'top') as 'top' | 'center' | 'bottom',
          font: (['inter', 'roboto', 'ptsans'].includes(String(block.font)) ? String(block.font) : 'inter') as 'inter' | 'roboto' | 'ptsans',
          fontSize: Number(block.font_size ?? 14),
          bold: Boolean(block.bold),
          italic: Boolean(block.italic),
        })),
      } satisfies PlayoffGrid
    } catch (error) {
      if (error instanceof ApiError && error.status === 404) return { cells: [], lines: [], textBlocks: [] }
      throw error
    }
  },
    ...
    await api(`/api/admin/playoff-grid/${tournamentId}/draft-validate`, {
      ...
      body: JSON.stringify({
        ...
        text_blocks: payload.textBlocks.map((block) => ({
          ...
          col: block.col,
          row: block.row,
          width_cells: block.widthCells,
          height_cells: block.heightCells,
          text: block.text,
          visible: block.visible,
          show_background: block.showBackground,
          align: block.align,
          vertical_align: block.verticalAlign,
          font: block.font,
