      ...
      <article className="rounded-2xl border border-borderSubtle bg-panelBg p-5 shadow-soft">
        ...
        {canManage && (
          <div className="mt-3 space-y-2 text-xs">
            <input type="file" accept="image/*" onChange={(e) => setImageFile(e.target.files?.[0] ?? null)} className="w-full rounded-lg border border-borderSubtle px-3 py-2" />
            ...
            <button
              ...
              onClick={async () => {
                ...
                if (!nextTitle) return
                try {
                  const imageUrl = imageFile ? (await uploadsRepository.uploadImage(imageFile)).url : event.imageUrl
