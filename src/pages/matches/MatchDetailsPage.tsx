...
export const MatchDetailsPage = () => {
  ...
  const teamStats = useMemo(() => {
    ...
    const pickRecent = (teamId: string) =>
      allMatches
        ...
        .slice(0, 3)
        .map((item) => ({ id: item.id, score: `${item.score.home}:${item.score.away}`, date: item.date }))
  ...
  const visibleComments = expandedChat ? comments : comments.slice(-5)
  ...
  return (
    <PageContainer>
      <section className="matte-panel overflow-hidden p-5">
        ...
        <div className="relative rounded-2xl bg-app/85 px-4 py-6">
          ...
          <div className="relative z-10 grid grid-cols-[1fr_auto_1fr] items-center gap-4">
            ...
            <div className="text-4xl font-bold tabular-nums text-accentYellow">
        ...
        <div className="mt-4 flex items-center gap-2">
          <button onClick={() => setLikes((prev) => prev + 1)} className="rounded-xl bg-elevated px-3 py-2 text-sm">👍 {likes}</button>
          <button onClick={() => setDislikes((prev) => prev + 1)} className="rounded-xl bg-elevated px-3 py-2 text-sm">👎 {dislikes}</button>
        </div>
              ...
              <div key={comment.id} className="rounded-xl bg-app/80 p-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {comment.isOwn && (
                      <button onClick={() => setComments((prev) => prev.filter((item) => item.id !== comment.id))} className="text-xs text-accentYellow">Удалить</button>
                    )}
                  </div>
                </div>
          ...
          <div className="mt-3 flex gap-2">
            ...
            <button
              onClick={() => {
                ...
                setComments((prev) => [...prev, { id: `c_${Date.now()}`, author: 'You', text: newComment.trim(), isOwn: true, createdAt: 'сейчас' }])
                setNewComment('')
              }}
      ...
      <div className="grid grid-cols-2 gap-2 text-sm">
        <Link className="matte-panel px-4 py-3 text-center text-textSecondary hover:text-textPrimary" to={`/teams/${match.homeTeamId}`}>Команда хозяев</Link>
        <Link className="matte-panel px-4 py-3 text-center text-textSecondary hover:text-textPrimary" to={`/teams/${match.awayTeamId}`}>Команда гостей</Link>
      </div>
    </PageContainer>
  )
}
