...
func (r *CommentsRepository) ListByEntity(ctx context.Context, entityType domain.CommentEntityType, entityID int64) ([]domain.Comment, error) {
	...
	SELECT c.id,c.entity_type,c.entity_id,c.parent_comment_id,c.author_user_id,c.body,c.edited_at,c.created_at,c.updated_at,c.deleted_at,
...
func (r *CommentsRepository) Create(ctx context.Context, c domain.Comment) (domain.Comment, error) {
	...
	VALUES ($1,$2,$3,$4,$5)
	RETURNING id,entity_type,entity_id,parent_comment_id,author_user_id,body,edited_at,created_at,updated_at,deleted_at`,
		c.EntityType, c.EntityID, c.ParentCommentID, c.AuthorUserID, c.Body)
	var out domain.Comment
	err := row.Scan(&out.ID, &out.EntityType, &out.EntityID, &out.ParentCommentID, &out.AuthorUserID, &out.Body, &out.EditedAt, &out.CreatedAt, &out.UpdatedAt, &out.DeletedAt)
	return out, err
}
...
func scanComment(row interface{ Scan(dest ...any) error }) (domain.Comment, error) {
	...
	err := row.Scan(&c.ID, &c.EntityType, &c.EntityID, &c.ParentCommentID, &c.AuthorUserID, &c.Body, &c.EditedAt, &c.CreatedAt, &c.UpdatedAt, &c.DeletedAt, &c.LikeCount, &c.DislikeCount)
	return c, err
}
